...
one of three tools per question: query_postgres, query_mongo, or
ask_clarification, so routing and query generation happen in one call.
"""
...
- Always use the exact table/column names or collection/field names given in the schemas."""
...
class ClaudeProvider(LLMProvider):
    ...
    # sends conversation history + both schema contexts to Claude, forced to pick one of the three tools
    def process_question(self, history: list, schema_context: str, mongo_schema_context: str = "") -> LLMResponse:
        messages = [{"role": turn["role"], "content": turn["content"]} for turn in history]
        combined_schema = f"Postgres schema:\n{schema_context}\n\nMongoDB schema:\n{mongo_schema_context}"
        ...
        for attempt in range(max_attempts):
            try:
                message = self.client.messages.create(
                    ...
                    tools=TOOLS,
                    tool_choice={"type": "any"},  # forces one of our tools, doesn't force which one
                    messages=messages,
                )
    ...
    # normalizes whichever tool Claude called into a single LLMResponse shape
